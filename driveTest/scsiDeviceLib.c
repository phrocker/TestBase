...
///////////////////////////////////////////////////////////////////////////
/** 
 *  @file       scsiDeviceLib.c
 *
 *  @brief      Simple device library to handle scsi devices interactions with
...
/////////////////////////////////////////////////////////////////////////////
/**
 *  @fn     int detectScsiDevices(char **ideDevices, char **ideDeviceNames)
 */
/////////////////////////////////////////////////////////////////////////////
int detectScsiDevices(char **ideDevices, char **ideDeviceNames)
{
        ...
        if (sscanf (buffer, "%i %2s", &deviceNumber,sstring)  == 2)
        {
            if (!strcmp(sstring,"sd"))
            {
                ...
                */
                ...
                deviceNumbers[deviceCount] = deviceNumber;
                
                snprintf(ideDeviceNames[deviceCount],MAX_DEVICE_STRING_LENGTH,"sd%i",deviceCount);
                deviceCount++;
            }
        }
    }
    ...
    while(fgets(buffer,sizeof(buffer),fd))
    {
        ...
        if (sscanf (buffer, "%i %i sd%c", &deviceNumber,&deviceCounter,&deviceCharacter)  ==3 )
        {
            
            if (deviceCounter == 0 && deviceNumber == deviceNumbers[i])
            {
                // populate the device location string
                snprintf(ideDevices[i],MAX_DEVICE_STRING_LENGTH,"/dev/sd%c",deviceCharacter);
